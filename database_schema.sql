...
CREATE TABLE IF NOT EXISTS `domainlist` (
    ...
    `status` TINYINT UNSIGNED DEFAULT 1 COMMENT '状态：1正常 2红色被封 3蓝色异常 4白色被封',
    `notify_status` TINYINT UNSIGNED DEFAULT 0 COMMENT '通知状态：0未通知 1正常 2红色被封 3蓝色异常 4白色被封',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `config` (
    ...
    `notify_types` VARCHAR(50) DEFAULT '2' COMMENT '通知类型：可多选，用逗号分隔，如2,3,4（2=红色被封，3=蓝色异常，4=白色被封）',
